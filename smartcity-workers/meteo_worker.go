...
func main() {
	...
	villesEnv := os.Getenv("METEO_VILLES")
	var villes []string
	if villesEnv == "" {
		villes = []string{"Clermont-Ferrand", "Aubiere", "Chamalieres", "Riom", "Aulnat"}
	} else {
		villes = strings.Split(villesEnv, ",")
	}
...
func getMeteo(ville string) (*MeteoData, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("Erreur HTTP %d: %s", resp.StatusCode, string(bodyBytes))
	}
